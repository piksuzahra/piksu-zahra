...
export default function Navbar({ activeSection }: { activeSection: string }) {
  ...
  return (
    <motion.nav 
      ...
      className="fixed bottom-10 left-1/2 -translate-x-1/2 gold-glass rounded-[var(--radius-premium)] z-50 w-[calc(100vw-3rem)] max-w-full md:max-w-max overflow-x-auto overflow-y-hidden custom-scrollbar border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
    >
      <div className="flex justify-between items-center px-6 py-4 min-w-max gap-8 md:gap-14">
      {menuItems.map((item) => {
        ...
        return (
          ...
          >
            <div className={`relative transition-all duration-700 ${isActive ? 'scale-110 -translate-y-1' : 'scale-100 group-hover:scale-110'}`}>
              <item.icon size={22} strokeWidth={isActive ? 2.5 : 1.5} className={isActive ? 'text-gold' : ''} />
              {isActive && (
                <motion.div 
