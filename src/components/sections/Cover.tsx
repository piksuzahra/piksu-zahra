...
export default function Cover({ onOpen }: { onOpen: () => void }) {
  ...
  const dayName = isNaN(d.getTime()) ? 'MINGGU' : new Intl.DateTimeFormat('id-ID', { weekday: 'long' }).format(d).toUpperCase();
  const monthName = isNaN(d.getTime()) ? 'JULI' : new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(d).toUpperCase();
    ...
    >
      ...
      {showLogin && (
        <div className="absolute inset-0 z-[200] bg-zinc-900/40 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="bg-white rounded-[var(--radius-premium)] p-8 shadow-2xl max-w-sm w-full relative" onClick={(e) => e.stopPropagation()}>
             <button onClick={() => setShowLogin(false)} className="absolute top-4 right-4 text-zinc-400 font-bold hover:text-zinc-800 transition-colors">
               <X size={20} />
             </button>
             <h3 className="font-serif text-3xl font-black text-center mb-8 tracking-tight italic">Admin Login</h3>
             <form onSubmit={(e) => {
               ...
               if (pass === '123456') {
                 if (['operator', 'couple-pria', 'couple-wanita', 'ortu-pria', 'ortu-wanita'].includes(user)) {
               ...
               <input name="username" type="text" placeholder="Username" className="w-full premium-input bg-zinc-50 border-zinc-100" />
               <input name="password" type="password" placeholder="Passcode" className="w-full premium-input bg-zinc-50 border-zinc-100" />
               <button type="submit" className="w-full premium-button bg-zinc-900 text-white font-black py-5 text-[10px] tracking-[0.3em] uppercase">Login</button>
             </form>
             
             <div className="mt-6 pt-6 border-t border-zinc-100">
               <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-widest text-center mb-4">Cloud Admin Access</p>
               <button 
                 onClick={async () => {
                   setIsLoggingIn(true);
                   try {
                 ...
                 className="w-full flex items-center justify-center gap-3 py-4 border-2 border-zinc-100 rounded-xl hover:bg-zinc-50 transition-colors disabled:opacity-50"
               >
                 <LogIn size={18} />
                 <span className="text-[10px] font-black uppercase tracking-widest">Sign in as Global Admin</span>
               </button>
             </div>
           </div>
        </div>
      ...
      <label className="absolute inset-0 cursor-pointer overflow-hidden z-0 block">
        <motion.div 
      ...
      <div className="relative z-10 flex flex-col items-center justify-between text-center px-6 w-full h-[100dvh] py-12 md:py-20 pointer-events-none">
        <div className="flex flex-col items-center mt-2 md:mt-6">
          ...
          >
             <div className="flex items-center gap-3 md:gap-4 mb-4">
                ...
                <p className="text-gold tracking-[0.4em] md:tracking-[0.7em] uppercase text-[7px] md:text-xs font-black drop-shadow-lg">
                  The Wedding of
                </p>
                <div className="w-6 md:w-12 h-px bg-gold/40"></div>
             </div>
          ...
          <motion.h1 
            ...
            className="font-script text-[70px] md:text-[150px] mb-8 md:mb-12 text-white drop-shadow-[0_15px_40px_rgba(0,0,0,0.8)] leading-[0.5] flex flex-col items-center select-none"
          >
            <motion.span 
              animate={{ y: [-2, 2, -2] }}
            ...
            >
              ...
              <span className="font-serif italic text-gold text-xl md:text-4xl drop-shadow-[0_4px_15px_rgba(212,175,55,0.6)]">&</span>
        ...
        >
          ...
          >
            ...
            {/* Hover shine effect */}
            <div className="absolute inset-0 bg-gold translate-y-full group-hover:translate-y-0 transition-transform duration-700 ease-[0.22,1,0.36,1]"></div>
            <div className="absolute -inset-2 opacity-0 group-hover:opacity-100 bg-gold/30 blur-2xl transition-opacity duration-700"></div>
          </motion.button>
        </motion.div>
      </div>
    </motion.div>
